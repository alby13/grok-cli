...
describe('checkNextSpeaker', () => {
  let chatInstance: GrokChatSession; // Type already updated
  ...
  beforeEach(() => {
    ...
    const mockConfigInstance = new MockConfig(
      'test-api-key',
      'grok-3-latest', // Updated model name
      false,
      '.',
      false,
      undefined,
    ...
    mockXaiClient = new XaiClient(mockConfigInstance); // Renamed class
    ...
    // GrokChatSession will receive the mocked instances via the mocked GoogleGenAI constructor
    // TODO: The mocking of GoogleGenAI and Models might need to change if GrokChatSession
    // directly uses XaiClient or GrokApiClient for its operations.
    chatInstance = new GrokChatSession( // Renamed class
      mockConfigInstance,
  ...
  it('should return null if history is empty', async () => {
    ...
    const result = await checkNextSpeaker(
      chatInstance,
      mockXaiClient, // Renamed variable
      abortSignal,
    );
    expect(result).toBeNull();
    expect(mockXaiClient.generateJson).not.toHaveBeenCalled(); // Renamed variable
  });
  ...
  it("should return { next_speaker: 'model' } when model intends to continue", async () => {
    ...
    (mockXaiClient.generateJson as Mock).mockResolvedValue(mockApiResponse); // Renamed variable
    ...
    expect(mockXaiClient.generateJson).toHaveBeenCalledTimes(1); // Renamed variable
  });
  ...
  it('should return null if xaiClient.generateJson throws an error', async () => {
    ...
    (mockXaiClient.generateJson as Mock).mockRejectedValue( // Renamed variable
      new Error('API Error'),
    );
  ...
  it('should return null if xaiClient.generateJson returns invalid JSON (missing next_speaker)', async () => {
    (chatInstance.getHistory as Mock).mockReturnValue([
      { role: 'model', parts: [{ text: 'Some model output.' }] },
    ] as Content[]);
    (mockXaiClient.generateJson as Mock).mockResolvedValue({ // Renamed variable
  ...
  it('should return null if xaiClient.generateJson returns a non-string next_speaker', async () => {
    (chatInstance.getHistory as Mock).mockReturnValue([
      { role: 'model', parts: [{ text: 'Some model output.' }] },
    ] as Content[]);
    (mockXaiClient.generateJson as Mock).mockResolvedValue({ // Renamed variable
  ...
  it('should return null if xaiClient.generateJson returns an invalid next_speaker string value', async () => {
    (chatInstance.getHistory as Mock).mockReturnValue([
      { role: 'model', parts: [{ text: 'Some model output.' }] },
    ] as Content[]);
    (mockXaiClient.generateJson as Mock).mockResolvedValue({ // Renamed variable
