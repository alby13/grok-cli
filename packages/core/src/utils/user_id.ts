...
const grokDir = path.join(homeDir, GROK_DIR); // Renamed
const userIdFile = path.join(grokDir, 'user_id'); // Renamed

function ensureGrokDirExists() { // Renamed
  if (!fs.existsSync(grokDir)) { // Renamed
    fs.mkdirSync(grokDir, { recursive: true }); // Renamed
  }
}
